...
#define SUPPORT_READ_1S_1S_1S_CMD_Pos (0UL)             /* The position of the Support for Read Command 1S-1S-1S
                                                          * in the JEDEC 4-byte Address Instruction Table, DWORD 1
                                                          */
#define SUPPORT_READ_1S_1S_1S_CMD_Msk (0x00000001UL)    /* The mask of the Support for Read Command 1S-1S-1S
                                                          * in the JEDEC 4-byte Address Instruction Table, DWORD 1
                                                          */
...
/* Defines to handle exit from 4Byte addressing mode */
#define SFDP_BFPT_BYTE_3D                     (0x3DU)                           /** Soft Reset and Rescue Sequence Support SFDP BFPT DW16 */
#define SFDP_BFPT_BYTE_3E                     (0x3EU)                           /** Exit 4Byte addressing info SFDP BFPT DW16 */

#define SFDP_EXIT_4_BYTE_METHOD_SOFT_RESET    (0x40U)                            /**< Soft reset supported mask in SFDP Basic Flash table DW16 */
#define SFDP_EXIT_4_BYTE_METHOD_HARD_RESET    (0x20U)                            /**< Hard reset supported mask in SFDP Basic Flash table DW16 */
#define SFDP_SOFT_RESET_COMMAND_SEQ_SUPPORTED (0x10U)                            /**< Soft reset using 66h and 99h mask in SFDP Basic Flash table DW16 */

#define SFDP_SOFT_RESET_ENABLE_CMD            (0x66U)
#define SFDP_SOFT_RESET_CMD                   (0x99U)
...
bool glSoftResetSupported = false;
...
static cy_en_smif_protocol_mode_t SfdpGetReadCmdParams(uint8_t const sfdpBuffer[],
                                                       ...
                                                       cy_stc_smif_mem_cmd_t* cmdRead);
static void SfdpGetReadFourBytesCmd_NormalRead(uint8_t const sfdpBuffer[],
...
static cy_en_smif_status_t SfdpCheckExitFourByteAddressing(SMIF_Type *base, uint8_t exitMethodByte,
                                                       uint8_t softResetRescueSeqByte,
                                                       cy_stc_smif_mem_device_cfg_t *device,
                                                       cy_en_smif_slave_select_t slaveSelect,
                                                       cy_stc_smif_context_t const *context);
...
#if BL_DEBUG
void Cy_App_PrintBuffer(uint8_t *buffer, uint32_t len)
{
...
'Pointer type conversion is intentional.')
    ...
    switch (protocolMode)
    {
        case PROTOCOL_MODE_1S_1S_1S:
            if (_FLD2BOOL(SUPPORT_READ_1S_1S_1S_CMD, sfdpForBytesTableDword1))
            {
                cmdRead->command = CY_APP_READ_4_BYTES_CMD_1S_1S_1S;
            }
            break;
        default:
                                                       ...
                                                       cy_stc_smif_context_t const *context)
{
    ...
    /* Check if Soft reset is a way to exit 4-byte addressing mode.
     * Bootloader only supports soft reset as a way to revert the 4 byte addressing mode.
     * If soft-reset is not supported, user application has to take care of
     * reverting the flash into 3B addressing mode or use 4Byte addressing mode for
     * user operation.*/

    if ((exitMethodByte & SFDP_EXIT_4_BYTE_METHOD_SOFT_RESET) != 0U)
    {
        /* Supports Soft Reset mode of exit */
        result = CY_SMIF_SUCCESS;

        if ((softResetRescueSeqByte & SFDP_SOFT_RESET_COMMAND_SEQ_SUPPORTED) != 0U)
        {
            glSoftResetSupported = true;
            DBG_APP_INFO("Soft reset supported\r\n",result);
        }
    }
    return result;
}

static cy_en_smif_status_t Cy_SFDP_DoSoftReset(SMIF_Type *base,
                                                       cy_en_smif_slave_select_t slaveSelect,
                                                       cy_stc_smif_context_t const *context)
{
    cy_en_smif_status_t result = CY_SMIF_CMD_NOT_FOUND;

    /* Send Reset Enable and Reset Command */
    Cy_SMIF_TransmitCommand( base, (uint8_t) SFDP_SOFT_RESET_ENABLE_CMD,
            CY_SMIF_WIDTH_SINGLE,
            NULL,
            CY_SMIF_CMD_WITHOUT_PARAM,
            CY_SMIF_WIDTH_NA,
    ...
    result = Cy_SMIF_TransmitCommand( base, (uint8_t) SFDP_SOFT_RESET_CMD,
            CY_SMIF_WIDTH_SINGLE,
            NULL,
            CY_SMIF_CMD_WITHOUT_PARAM,
            CY_SMIF_WIDTH_NA,
...
cy_en_smif_status_t Cy_SFDP_ExitFourByteAddressingMode(SMIF_Type *base,
                                                       cy_en_smif_slave_select_t slaveSelect,
                                                       cy_stc_smif_context_t const *context)
{
    cy_en_smif_status_t result = CY_SMIF_CMD_NOT_FOUND;
    if(glSoftResetSupported)
    {
        result = Cy_SFDP_DoSoftReset(base, slaveSelect, context);
    }
        ...
        if (basicSpiTableLength > 0U)
        {
            ...
            if (CY_SMIF_SUCCESS == result)
            {
                ...
                if (FOUR_BYTE_ADDRESS == device->numOfAddrBytes)
                {
                    ...
                    /* Check and save how to exit 4-byte addressing mode */
                    SfdpCheckExitFourByteAddressing(base, sfdpBuffer[SFDP_BFPT_BYTE_3E],sfdpBuffer[SFDP_BFPT_BYTE_3D], device, slaveSelect, context);
                	...
                	SfdpGetReadFourBytesCmd_NormalRead(fourByteAddressBuffer, pMode, cmdRead);
                                    ...
                                    cy_stc_smif_context_t *context)
{
    ...
    /* Bootloader supports max x1 SPI Mode */
    return Cy_SMIF_MemInitSfdpMode_Lite(base, &memSfdpDetect, CY_SMIF_WIDTH_SINGLE, CY_SMIF_SFDP_QER_0, context);
}
